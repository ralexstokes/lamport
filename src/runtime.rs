...
use crate::{
    ...
    envelope::{
        CallTimedOut, DownMessage, Envelope, ExitSignal, Message, ReplyToken, TaskCompleted,
        TimerFired,
    },
    internal::{
...
trait ActorCell: Send {
    ...
    fn shutdown(&mut self, ctx: &mut LocalContext<'_>) -> ActorTurn;
...
impl SupervisorContext for LocalContext<'_> {
    fn configure_supervisor(&mut self, flags: SupervisorFlags, child_specs: Vec<ChildSpec>) {
        match self.current.supervisor.as_mut() {
...
impl LocalRuntime {
    ...
    /// Upgrades a booted application's root supervisor tree through the local control plane.
    ...
    fn cancel_call_timeout(&mut self, actor: ActorId, reference: Ref) -> bool {
        let mut cancelled = false;
        if let Some(handle) = self.call_timeout_tasks.remove(&(actor, reference)) {
            handle.abort();
            cancelled = true;
        }
...
mod tests {
    ...
    use crate::{
        Actor, ActorTurn, CallOutcome, Context, ControlError, Envelope, ExitReason, GenServer,
        LifecycleEvent, LocalRuntime, PoolKind, ReceivedEnvelope, Ref, RegistryError, ReplyToken,
        SchedulerConfig, SendError, ServerOutcome, Shutdown, SpawnError, SpawnOptions,
    ...
    fn managed_shutdown_still_succeeds_when_runtime_reserve_is_exhausted() {
        ...
        runtime.send(actor, 1_u32).unwrap();
        runtime
            .send_system(actor, SystemMessage::Suspend)
            .expect("reserved slot should admit a system message");
    ...
    fn runtime_owned_delivery_overflow_forces_exit() {
        ...
        assert!(runtime.enqueue_runtime_envelope(
            actor,
    ...
    fn cancel_call_timeout_withdraws_queued_timeout_envelope() {
        ...
        let state = runtime.actor_state_mut(actor).unwrap();
        super::push_envelope(state, Envelope::call_timeout(reference)).unwrap();
        assert_eq!(state.mailbox.len(), 1);

        assert!(runtime.cancel_call_timeout(actor, reference));

        let state = runtime.actor_state_mut(actor).unwrap();
        assert!(state.mailbox.is_empty());
        assert_eq!(state.metrics.mailbox_len, 0);
    }
