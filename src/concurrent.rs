...
mod tests {
    ...
    fn shutdown_bypasses_selective_receive_with_backlogged_mailbox() {
        ...
        while Instant::now() < deadline {
            ...
            if snapshot.status == ActorStatus::Waiting && snapshot.metrics.mailbox_len == 2 {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        ...
        let snapshot = runtime.actor_snapshot(actor).unwrap();
        assert_eq!(snapshot.status, ActorStatus::Waiting);
        ...
        while Instant::now() < deadline {
            let snapshot = runtime.actor_snapshot(actor).unwrap();
            if snapshot.status == ActorStatus::Dead {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
    ...
    fn code_change_bypasses_selective_receive_with_pending_call() {
        ...
        while Instant::now() < deadline {
            let snapshot = runtime.actor_snapshot(actor).unwrap();
            if snapshot.status == ActorStatus::Waiting && snapshot.metrics.mailbox_len == 1 {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
    ...
    fn trace_options_all_capture_mailbox_depth_and_scheduler_metadata() {
        ...
        let deadline = Instant::now() + Duration::from_secs(1);
        while Instant::now() < deadline {
            if runtime.actor_snapshot(actor).unwrap().suspended {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        ...
        while Instant::now() < deadline {
            let snapshot = runtime.actor_snapshot(actor).unwrap();
            if snapshot.suspended && snapshot.metrics.mailbox_len == 2 {
                break;
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        ...
        let received = runtime.event_log().into_iter().filter(|event| {
            matches!(
                &event.kind,
                RuntimeEventKind::Trace(trace)
                    if trace.actor == actor
                        ...
                        && trace.mailbox_len.is_some_and(|mailbox_len| mailbox_len >= 1)
                        && trace.scheduler_id.is_some()
            )
        });
